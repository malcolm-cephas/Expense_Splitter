...
function App() {
  ...
  // Modal/Form States
  const [activeModal, setActiveModal] = useState(null); // 'ADD_MEMBER', 'ADD_EXPENSE', 'STATS', 'SETTLE_UP'
  ...
  const handleSetFamilyName = async (member) => {
  ...
  const handleExportPDF = () => {
    ...
    doc.autoTable({ startY: 25, head: [['Date', 'Description', 'Category', 'Amount']], body: data });
    ...
    <div className="app-border-pane" onContextMenu={(e)=>e.preventDefault()}>
      ...
      <div className="main-body">
        <div className="sidebar" onContextMenu={(e)=>e.preventDefault()}>
          ...
          <h4>New Group</h4>
          <input className="fx-input" placeholder="Name" value={newGroupName} onChange={e=>setNewGroupName(e.target.value)} />
          <input className="fx-input" placeholder="Budget" value={newGroupBudget} onChange={e=>setNewGroupBudget(e.target.value)} />
            ...
            <div className="flex-col h-full gap-20">
              <div className="flex-row items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-muted m-0">{selectedGroup.description || `${selectedGroup.members.size} members`}</p>
                </div>
                <div className="flex-row gap-10">
                  ...
                  <button className="fx-button accent" onClick={()=>{
                    ...
                    setExpPayerId(selectedGroup.members[0].id); setInvolvedMembers(selectedGroup.members.map(m=>m.id));
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <div className="modal-body">
              <input className="fx-input" placeholder="Email" value={memberEmail} onChange={e=>setMemberEmail(e.target.value)} />
            </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content w-450">
            ...
            <div className="modal-body flex-col gap-10">
              <input className="fx-input" placeholder="Desc" value={expDesc} onChange={e=>setExpDesc(e.target.value)} />
              <input className="fx-input" type="number" placeholder="Amt" value={expAmt} onChange={e=>setExpAmt(e.target.value)} />
              <select className="fx-input" value={expCat} onChange={e=>setExpCat(e.target.value)}>
                <option>Food</option><option>Travel</option><option>Utilities</option><option>Entertainment</option><option>Other</option>
              </select>
              <input className="fx-input" type="date" value={expDate} onChange={e=>setExpDate(e.target.value)} />
              <label>Paid By:</label>
              <select className="fx-input" value={expPayerId} onChange={e=>setExpPayerId(e.target.value)}>
          ...
          <div className="modal-content w-600">
            ...
            <div className="modal-body h-400">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={(() => {
                    const m = {}; selectedGroup.expenses?.forEach(e => m[e.category] = (m[e.category] || 0) + e.amount);
                    return Object.keys(m).map(n => ({ name: n, value: m[n] }));
                  })()} outerRadius={120} label={({name, percent})=>`${name} ${(percent*100).toFixed(0)}%`} dataKey="value">
