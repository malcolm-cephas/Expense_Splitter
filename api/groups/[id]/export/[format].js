    ...
    try {
        ...
        if (format === 'csv') {
            const headers = [
                ...
                'Payers',
                'Split Type',
                ...group.members.map((m) => `Owed by ${m.userId.name}`)
            ];
            const rows = expenses.map((exp) => {
                const payersInfo = exp.payers.map((p) => {
                    const m = group.members.find((gm) => gm.userId._id.toString() === p.userId.toString());
                    return `${m?.userId.name || 'Unknown'}: ${p.amount}`;
                }).join('; ');
                const memberOwed = group.members.map((m) => {
                    const split = exp.splits.find((s) => s.userId.toString() === m.userId._id.toString());
